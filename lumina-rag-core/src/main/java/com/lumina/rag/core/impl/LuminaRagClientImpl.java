        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                String finalAnswer = deduplicator.execute(deduplicationKey, () -> {
                    ...
                    // B. 拼装 CoT 模板
                    String context = chunks.stream().map(DocumentChunk::getText).collect(Collectors.joining("\n---\n"));
                    ...
                    streamingChatModel.generate(prompt, new StreamingResponseHandler<AiMessage>() {
                        ...
                        public void onComplete(Response<AiMessage> response) {
                            ...
                            cacheManager.putCache(query, queryVector, fullResponse.toString(), refDocIds);
                            // 瞬间唤醒在门外挂起的跟随者
